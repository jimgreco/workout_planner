...
// Mock the store module — functions now take userId as first param
vi.mock('../store', () => ({
  saveExercise: vi.fn((_userId, ex) => {
...
describe('Exercises page', () => {
  it('renders exercise names', () => {
    render(<Exercises userId={UID} exercises={sampleExercises} onUpdate={() => {}} />);
  ...
  it('shows empty state when no exercises', () => {
    render(<Exercises userId={UID} exercises={[]} onUpdate={() => {}} />);
  ...
  it('calls onUpdate and closes modal on save', () => {
    ...
    render(<Exercises userId={UID} exercises={sampleExercises} onUpdate={onUpdate} />);
    fireEvent.click(screen.getByText('+ Add Exercise'));
    fireEvent.change(screen.getByPlaceholderText(/e.g. Bench Press/i), { target: { value: 'Pull Up' } });
